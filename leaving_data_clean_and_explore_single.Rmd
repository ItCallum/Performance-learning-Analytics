...
reasons_precenatge <- as.data.frame(table(combine_Leaving_NoDups$leaving_reason)) %>% mutate(percent = (Freq/sum(Freq)) * 100 ) 

ggplot(reasons_precenatge, aes(x="", y = percent, fill=Var1)) + 
...
head(combine_Leaving_NoDups)

 (na.omit(combine_Leaving_NoDups)) %>% filter(last_completed_week_number < last_completed_week_number) 
...
table(combine_Leaving_clean$last_completed_week_number)

ggplot(combine_Leaving_clean, aes(last_completed_week_number)) + geom_bar() + facet_wrap(~ Cycle, ncol = 2) + ggtitle("The amount of leavers last completed week")

print("--last_completed_step--")
table(combine_Leaving_clean$last_completed_step)

 combine_Leaving_clean_Remove_Zero <- combine_Leaving_clean %>% filter(last_completed_step_number != 0)

 combine_Leaving_clean_Remove_Zero
 
ggplot(combine_Leaving_clean_Remove_Zero, aes(last_completed_step)) + geom_bar() + ggtitle("The amount of leavers last completed step")
ggplot(combine_Leaving_clean_Remove_Zero, aes(last_completed_step)) + geom_bar() + facet_wrap(~ Cycle, ncol = 2) + ggtitle("The amount of leavers last completed step per cycle")
``` 
...
combine_Leaving_clean %>% filter(Cycle == 4) %>%  ggplot(aes(x=Date, y=Time, color=last_completed_week_number)) + geom_point() +  geom_vline(xintercept = as.Date("2017-11-13"))
combine_Leaving_clean %>% filter(Cycle == 5) %>%  ggplot(aes(x=Date, y=Time, color=last_completed_week_number)) + geom_point() +  geom_vline(xintercept = as.Date("2018-02-05"))
combine_Leaving_clean %>% filter(Cycle == 6) %>%  ggplot(aes(x=Date, y=Time, color=last_completed_week_number)) + geom_point() +  geom_vline(xintercept = as.Date("2018-06-11"))
combine_Leaving_clean %>% filter(Cycle == 7) %>%  ggplot(aes(x=Date, y=Time, color=last_completed_week_number)) + geom_point() + geom_vline(xintercept = as.Date("2018-09-10"))
...
combine_Leaving_clean %>% filter(Cycle == 6 & Date < as.Date("2018-06-11") )

combine_Leaving_clean %>% filter(learner_id == "353eaf1b-cff8-4d9c-98d4-b9a736c17e73") 
...
combine_Leaving_clean %>% filter(left_at < last_completed_step_at  )
