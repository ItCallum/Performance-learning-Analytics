...
- These people have left the course before they had done anything on the course so a week value wasn't recorded
...
#### The day people left
Another thing i wanted to look was left_at, This is moment in time where a person left the course.
...
```{r leaving_reason_lcwn_percenatge, echo=FALSE, fig.height = 3}
...
```{r reason_Leave_32, echo=FALSE}
Reason_Step_32 
```
Looking at the reason given we see that the reason is overwhelimily "other". This suggest that there is a really unique issue with this video that is causing people to leave. Maybe its offending people?
...
1279 users who left the course which is alot more than those who gave a leaving response. However the total size of the combined enrollment data was `r nrow(combine_Enrolments) ` meaning we have `r nrow(combine_Enrolments) - 1279` rows with no unenrollment data.
